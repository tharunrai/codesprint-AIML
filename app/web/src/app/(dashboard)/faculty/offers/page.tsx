...
import { formatCTC, type OfferStatus } from "@/lib/mock-data";

export default function FacultyOffersPage() {
  const { offerLetters, reviewOffer } = usePlacement();
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-1.5 p-1 bg-surface border border-border rounded-xl overflow-x-auto">
            {(["all", "received", "uploaded", "verified", "accepted", "declined"] as const).map((tab) => (
              <button
                key={tab}
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        <Button
                          variant="danger"
                          size="sm"
                          onClick={() => reviewOffer(offer.id, "received", currentRemark)}
                        >
                          Reject
                        </Button>
                        <Button
                          ...
                          onClick={() => reviewOffer(offer.id, "verified", currentRemark)}
                        >
                          Approve & Verify
                        </Button>
                      </div>
